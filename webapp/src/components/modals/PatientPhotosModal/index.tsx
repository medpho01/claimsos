...
const PatientPhotosModal: React.FC<PatientPhotosModalProps> = ({ patient, onClose, onUpdate }) => {
  ...
  // --- Data Hooks ---
  const { photosData, loading, error, isCached, isDeleting, fetchPhotos, deleteFiles } =
    usePhotosData(patient.id, patient.admission_type);
  ...
  const downloadFile = async (file: MediaFile) => {
    try {
      ...
      if (file.mimeType !== "application/pdf" && file.mimeType.startsWith("image/")) {
        ...
        link.href = imageData;
        link.download = fileName.split(".")[0] + ".jpeg";
  ...
  const handleSaveDetails = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onUpdate(patient.id, {
        ...
        claimApproved: claimsValues.claimApproved
          ? parseFloat(claimsValues.claimApproved)
          : undefined,
    ...
    <Dialog open={true} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        {mainTab === "photos" &&
          !loading &&
          !error &&
          (photosData?.categories?.length || 0) > 0 &&
          !isSelectMode && (
          ...
          {mainTab === "photos" ? (
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
              <form
                ...
                className="max-w-4xl mx-auto bg-white p-6 rounded-xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6"
              >
                {mainTab === "ipd" ? (
        ...
        {selectedPhoto && (
          <Lightbox
            ...
            hasNext={
              getActivePhotos.findIndex((p) => p.id === selectedPhoto.id) <
