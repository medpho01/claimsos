 ...
 * @param candidateCategories The 21 doc_category codes (passed in to keep the
 *                     prompt in lock-step with master_options at runtime).
 */
export function buildDocClassifierUserPrompt(input: {
  ...
  const { sectionText, pagesContext, candidateCategories } = input;
